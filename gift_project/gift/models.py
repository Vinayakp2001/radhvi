...
class Order(models.Model):
    ...
    @property
    def payment_gateway_used(self):
        """Determine which payment gateway was used for this order"""
        if self.phonepe_merchant_transaction_id:
            return 'phonepe'
    ...
    @property
    def is_phonepe_payment(self):
        """Check if this order used PhonePe payment"""
        return bool(self.phonepe_merchant_transaction_id)
    
    @property
    def is_razorpay_payment(self):
        """Check if this order used Razorpay payment"""
        return bool(self.razorpay_order_id)
    
    @property
    def payment_transaction_id(self):
        """Get the transaction ID regardless of payment gateway"""
        if self.phonepe_transaction_id:
            return self.phonepe_transaction_id
        elif self.razorpay_payment_id:
    ...
    def payment_gateway_display_name(self):
        """Get display name for the payment gateway used"""
        gateway = self.payment_gateway_used
        if gateway == 'phonepe':
            return 'PhonePe'
    ...
    def update_phonepe_payment_details(self, payment_data):
        """
        Update order with PhonePe payment details
        
        Args:
            payment_data: Dictionary with PhonePe payment information
        """
        self.phonepe_merchant_transaction_id = payment_data.get('merchant_transaction_id', '')
        self.phonepe_transaction_id = payment_data.get('transaction_id', '')
        self.phonepe_payment_instrument = payment_data.get('payment_instrument', '')
        self.phonepe_response_code = payment_data.get('response_code', '')
        
        # Update general payment fields
        if payment_data.get('transaction_id'):
            self.transaction_id = payment_data['transaction_id']
        ...
        elif payment_data.get('payment_status') == 'FAILED':
    ...
    def get_payment_details(self):
        """
        Get payment details in a unified format regardless of gateway
        
        Returns:
            Dictionary with payment information
        """
        if self.is_phonepe_payment:
            return {
                'gateway': 'phonepe',
                'gateway_display': 'PhonePe',
                'transaction_id': self.phonepe_transaction_id,
                'merchant_transaction_id': self.phonepe_merchant_transaction_id,
                'payment_instrument': self.phonepe_payment_instrument,
                'response_code': self.phonepe_response_code,
                'payment_status': self.payment_status,
                'payment_date': self.payment_date,
            }
        elif self.is_razorpay_payment:
            return {
                'gateway': 'razorpay',
                'gateway_display': 'Razorpay',
                'order_id': self.razorpay_order_id,
                'payment_id': self.razorpay_payment_id,
                'signature': self.razorpay_signature[:20] + '...' if self.razorpay_signature else '',
                'payment_status': self.payment_status,
                'payment_date': self.payment_date,
            }
        else:
    ...
    @property
    def payment_transaction_id(self):
        """Get the transaction ID based on payment gateway used"""
        if self.phonepe_transaction_id:
            return self.phonepe_transaction_id
        elif self.razorpay_payment_id:
            return self.razorpay_payment_id
    ...
    @property
    def is_phonepe_payment(self):
        """Check if this order was paid using PhonePe"""
        return bool(self.phonepe_merchant_transaction_id)
    
    @property
    def is_razorpay_payment(self):
        """Check if this order was paid using Razorpay"""
        return bool(self.razorpay_order_id)
    
    def get_payment_details(self):
        """Get payment details based on gateway used"""
        if self.is_phonepe_payment:
            return {
                'gateway': 'phonepe',
                'merchant_transaction_id': self.phonepe_merchant_transaction_id,
...
class Address(models.Model):
    ...
    def clean(self):
        ...
        pincode_digits = ''.join(filter(str.isdigit, self.pincode))
        ...
        if len(phone_digits) < 10:
            raise ValidationError({'phone': 'Phone number must contain at least 10 digits'})
...
# ============================================================================
# MODELS FOR NEXT.JS FRONTEND
# ============================================================================

class Occasion(models.Model):
    """
    Occasions for 'Shop by Occasion' section on homepage
    Examples: Birthday, Anniversary, Wedding, Valentine's Day, etc.
    """
    name = models.CharField(max_length=100)
    slug = models.SlugField(unique=True)
    tagline = models.CharField(max_length=200, help_text="Short tagline like 'Make birthdays unforgettable'")
    description = models.TextField(blank=True)
    ...
    @property
    def product_count(self):
        """Count products in categories linked to this occasion"""
        return Product.objects.filter(category__name__icontains=self.name).count()


class Testimonial(models.Model):
    """
    Customer testimonials for homepage display
    """
    name = models.CharField(max_length=100, help_text="Customer name")
    city = models.CharField(max_length=100, help_text="Customer city")
    text = models.TextField(help_text="Testimonial text")
    tag = models.CharField(
        max_length=50,
        help_text="e.g., Birthday Gift, Anniversary Gift, Wedding Gift"
    )
