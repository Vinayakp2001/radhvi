...
def home(request):
    ...
    featured_products = Product.objects.filter(is_featured=True).prefetch_related('images')[:8]
    trending_products = Product.objects.filter(is_trending=True).prefetch_related('images')[:8]
    best_sellers = Product.objects.filter(is_best_seller=True).prefetch_related('images')[:8]
    new_arrivals = Product.objects.filter(is_new_arrival=True).prefetch_related('images')[:8]
    deal_of_day = Product.objects.filter(is_deal_of_day=True).prefetch_related('images').first()
    
    # Hot Deal Products - You can create these products in admin
    earring_deal = Product.objects.filter(slug='premium-oxidized-earring-collection').prefetch_related('images').first()
    roses_deal = Product.objects.filter(slug='luxury-80-red-roses-bouquet').prefetch_related('images').first()
    ...
    context = {
        ...
        'earring_deal': earring_deal,
        'roses_deal': roses_deal,
