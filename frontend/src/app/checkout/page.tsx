...
export default function CheckoutPage() {
  ...
  useEffect(() => {
    ...
    if (!hasCart && !hasGuestCart) { router.push('/cart'); return; }
  ...
  const handleOrder = async () => {
    ...
    try {
      ...
      if (paymentMethod === 'cod') router.push('/orders/' + order_id + '/confirmation');
      else { localStorage.setItem('pending_order_id', order_id); window.location.href = payment_url; }
    } catch (err: any) {
          ...
          <div className="lg:col-span-2 space-y-4">
            <div className="bg-white rounded-xl border border-gray-200 overflow-hidden">
              <div className="px-6 py-4 border-b border-gray-100 flex items-center justify-between">
                <h2 className="font-semibold text-gray-900">{step === 1 ? 'Your Details' : 'Delivery Address'}</h2>
                ...
                <div className="p-6 space-y-4">
                  {!isAuthenticated && (
                    <div className="bg-blue-50 border border-blue-100 rounded-lg p-4 space-y-3">
                      <p className="text-sm font-medium text-blue-800">Create account or <button onClick={() => router.push('/login?redirect=/checkout')} className="underline">sign in</button></p>
                    ...
                    <div className="space-y-2">
                      ...
                      {savedAddresses.map((a: any) => (
                        <label key={a.id} className={'flex items-start gap-3 p-3 border rounded-lg cursor-pointer ' + (selectedAddressId === a.id ? 'border-red-500 bg-red-50' : 'border-gray-200')}>
                          ...
                          <div><p className="text-sm font-medium">{a.full_name || a.name}</p><p className="text-xs text-gray-500">{a.address_line1}, {a.city}, {a.state} - {a.pincode}</p></div>
                        </label>
                      ))}
                  ...
                  {(!isAuthenticated || !selectedAddressId) && (
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                      <div><label className="block text-sm font-medium text-gray-700 mb-1">Full Name *</label><input value={addr.name} onChange={e => setAddr(p => ({ ...p, name: e.target.value }))} className={inp('name')} />{errors.name && <p className="text-red-500 text-xs mt-1">{errors.name}</p>}</div>
                      <div><label className="block text-sm font-medium text-gray-700 mb-1">Phone *</label><input value={addr.phone} onChange={e => setAddr(p => ({ ...p, phone: e.target.value }))} className={inp('phone')} />{errors.phone && <p className="text-red-500 text-xs mt-1">{errors.phone}</p>}</div>
                      <div className="md:col-span-2"><label className="block text-sm font-medium text-gray-700 mb-1">Address *</label><textarea value={addr.address} onChange={e => setAddr(p => ({ ...p, address: e.target.value }))} rows={2} className={inp('address') + ' resize-none'} />{errors.address && <p className="text-red-500 text-xs mt-1">{errors.address}</p>}</div>
                      <div><label className="block text-sm font-medium text-gray-700 mb-1">Pincode *</label><div className="relative"><input value={addr.pincode} onChange={e => setAddr(p => ({ ...p, pincode: e.target.value }))} onBlur={handlePincodeBlur} maxLength={6} className={inp('pincode')} />{pincodeLoading && <div className="absolute right-3 top-1/2 -translate-y-1/2 animate-spin rounded-full h-4 w-4 border-2 border-red-500 border-t-transparent" />}</div>{errors.pincode && <p className="text-red-500 text-xs mt-1">{errors.pincode}</p>}</div>
                      <div><label className="block text-sm font-medium text-gray-700 mb-1">City *</label><input value={addr.city} onChange={e => setAddr(p => ({ ...p, city: e.target.value }))} className={inp('city')} />{errors.city && <p className="text-red-500 text-xs mt-1">{errors.city}</p>}</div>
                      <div><label className="block text-sm font-medium text-gray-700 mb-1">State *</label><input value={addr.state} onChange={e => setAddr(p => ({ ...p, state: e.target.value }))} className={inp('state')} />{errors.state && <p className="text-red-500 text-xs mt-1">{errors.state}</p>}</div>
                    </div>
                  )}
                  <button onClick={() => { if (validate()) setStep(2); }} className="w-full mt-2 bg-red-500 text-white py-3 rounded-lg font-medium hover:bg-red-600">Continue to Payment</button>
                </div>
              ) : (
                ...
                <div className="p-6">
                  ...
                  <div className="space-y-2 mb-6">
                    ...
                    <label className={'flex items-center gap-3 p-4 border rounded-lg cursor-pointer ' + (paymentMethod === 'phonepe' ? 'border-red-500 bg-red-50' : 'border-gray-200')}>
                      ...
                      <div><p className="text-sm font-medium">Pay Online</p><p className="text-xs text-amber-600">Currently unavailable</p></div>
                    </label>
                  </div>
                  ...
                  <p className="text-xs text-gray-400 text-center mt-3">By placing this order, you agree to our Terms and Conditions</p>
                </div>
              </div>
            )}
          </div>
                ...
                <div className="space-y-3 mb-4 max-h-60 overflow-y-auto">
                  {cart?.items.map((item: any) => (
                    <div key={item.id} className="flex gap-3">
                      <div className="w-14 h-14 bg-gray-100 rounded-lg flex-shrink-0 overflow-hidden">{item.product.image_url ? <img src={item.product.image_url} alt={item.product.name} className="w-full h-full object-cover" /> : <div className="w-full h-full bg-gray-200" />}</div>
                      <div className="flex-1 min-w-0"><p className="text-sm font-medium text-gray-900 truncate">{item.product.name}</p><p className="text-xs text-gray-500">Qty: {item.quantity}</p><p className="text-sm font-semibold">Rs.{Number(item.item_total).toFixed(2)}</p></div>
                    </div>
                  ))}
                  {!isAuthenticated && guestCart.length > 0 && (
                    <p className="text-xs text-gray-500">{guestCart.length} item(s) in your cart</p>
                  )}
                </div>
                <div className="border-t pt-4 space-y-2">
                  <div className="flex justify-between text-sm text-gray-600"><span>Subtotal</span><span>Rs.{Number(cart?.total_amount || 0).toFixed(2)}</span></div>
                  <div className="flex justify-between text-sm text-gray-600"><span>Shipping</span><span>Calculated at checkout</span></div>
                  <div className="flex justify-between font-semibold text-gray-900 pt-2 border-t"><span>Total</span><span>Rs.{total.toFixed(2)}</span></div>
                </div>
              </div>
            </div>
          </div>
